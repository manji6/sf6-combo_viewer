...
//    ただし manon-jump-jhk-hoseigiri では、構造的に全く同じ位置（J強K→4強P→OD
//    グランフェッテ→4強P→強ロン・ポワン、段5・即時補正-10pt適用中）にも
//    関わらず、強ロン・ポワンの実測が 480（80%基礎値×60%、即時補正が掛からず
//    半分にもならない）で、odfouette 側の 200（半分・即時補正あり）と食い違う
//    （両方ともオーナーからの実測で「浮き状態ヒットだから1回」という同じ説明）。
//    原因未特定のため、このアプリの計算では従来どおり（半分にせず即時補正を
//    適用する）扱いのままにしてあり、jump-jhk-hoseigiri 側は差分を許容している。
