...
コマンド・ダメージ・フレーム・起き攻けの中身は正確ではありません。UI と操作感の確認が目的です。
...
| **コンボフロー** | `/manon/combos/manon-mid-5mp-drc-ranversement/` | 1 手＝1 ノードで左→右。別の締め（SA3）が分岐、締めのダウンから破線で起き攻け。ツールバーの**クラシック/モダン切替**（このコンボは両対応）と**全画面** |
...
| パーツ詳細 | `/manon/routes/oki_dr2mk_from_degage_light/` | 択の特徴（frameAdvantage / vsWakeup 含む）、手順のクラシック/モダン小トグル、周辺の相関 |
...
2. `src/data/dummy/routes.ts` … 実例が一番豊富（starter / ender / okizeme＋properties / 非DR始動 / クラシック限定）
3. `src/data/dummy/situations.ts` … 状況ノード（advantage / wakeupNote）
4. `src/data/dummy/combos.ts` … コンボは routeChain（パーツID列）だけ
5. `src/lib/graph/derive.ts` の `validateAll()` … 守るべき整合性ルール

Phase 2 でこれらは `src/content/{situations,routes,combos}/manon/*.json` ＋ Zod に移行（**構造は不変**、TS 配列 → JSON ファイル群）。
