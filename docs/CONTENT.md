...
- **技コマンドでない操作**は `action` を付ける（`command` はダミーでよい）:
  - `walk` 微歩き / `walk_back` 様子見（微後ろ歩き） / `dash` 前ステップ（`66`）/ `dash_back` バックステップ / `whiff` 空振り / `feint` フェイント（ボタンホールド派生など） / `wait` 何もしない
  - `action` 付き step はフレームを出さない。`whiff` / `feint` は「＜技名＞ 空振り／フェイント」と出すので `move` と `moveKey` を付ける。他は `moveKey` 不要
...
- `combo.damageOverride` には実測値を入れる（メダル依存の技があればメダル1枚時の値）
- `combo.damageNote` は**メダル依存があるコンボだけ** `"メダル保持数でダメージが変動"` の1文（数値は入れない。実測前/素点合計などの言葉も書かない）。固定ダメージのコンボには付けない
- UI: 一覧カードはダメージに `±` を付け注記を1行、詳細ページはダメージ値の下に注記
- 対象の技: `manon-manege-l/h/od`・`manon-ranversement-l/m/h/od`・`manon-sa3`（グラン・フェッテ自体は固定 800）
- **公開文言のルール**: 出典・「オーナー実機確認」・「実測」「素点合計」などの制作メモは JSON に残さない（サイトに表示される）。数値だけ書く

### 6.2 補正切り（damage scaling reset）

コンボが一旦切れると SF6 のコンボ補正（ヒットごとの逓減）がリセットされ、次の単発は 100% ダメージで入る。
**強ランヴェルセなどをあえて空振り（フレーム消費）してコンボを終わらせ、相手の起き上がりに単発
（多くはコマ投げ）を重ねる**連携を「補正切り」と呼ぶ。伸ばすより火力が出ることがある。

- コンボを切る手段は 2 通り。step の `action` で表す（`moveKey` も付ける）
  - `whiff`（空振り）: 例 強ランヴェルセを当てずにスカす → 相手はダウン → `kd_after_hoseigiri_mid`（kind `knockdown`）
  - `feint`（フェイント）: 例 中ランヴェルセのコマンド最後の P をホールドして派生 → +1F → `kd_after_hoseigiri_feint_mid`（kind `hit`, `advantage` "+1"）
  - どちらも「＜技名＞ 空振り／フェイント」と表示され、フレームは出ない
- ルート分岐を見せたいコンボ（切った後に 2択以上ある）は **routeChain = [setup のみ]、`endAt` = 補正切りノード**にする。
  そのノードから複数の `okizeme` ルートを生やすと、コンボフローが破線で扇状に分岐表示される
- 分岐が1つだけなら routeChain = [setup, 締めの okizeme] の2ルートでよい
- Route（切った後の単発）: `constraints` に「補正切りで100%」、`properties.weakVs` に 垂直/無敵技/DI（＝確定ではない読み合い）
- Combo: `damageOverride` = 合計、`damageNote` = メダル依存なら `"メダル保持数でダメージが変動"`、tag `補正切り`
