...
| DC-7 | モダン簡易入力の補正計算 | オーナー＋開発 | DC-3 | 🔄 | `controlType:'modern'` を実装済み（SPボタン簡易入力×0.8、MECHANISM.md §4.9）。`Move.inputModern` が未確認の技（SA1〜3・CA 全て）を含むコンボは incomplete になる。実測値による裏取りはまだ無い |

DC-6（公開表示への接続）は、対象コンボがある程度揃ってからオーナー判断で着手する。
DC-7 の残作業: 各技の `inputModern`（モダンのSP簡易入力コマンド）を公式コマンドリストで確認して埋める（特にSA1〜3・CA）。可能ならオーナーの対戦画面でモダン実測値を確認して裏取りする。
