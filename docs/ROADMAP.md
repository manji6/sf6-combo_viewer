...
| P2-4a | 代表実例の登録・検証（RV-02〜04） | 🔄 batch1 投入済み | **batch1（`1e7359d`）**: すこれるブログのコア6コンボ＋弱デガジェ〆起き攻め（moves16/situations7/routes11/combos6）。出典明記、技フレームは公式。**オーナー実機確認待ち** → ダメージ測定・ゲージ内訳・モダン割り当ての確定 → P2-1 に確認状態フィールド追加。以降 batch2〜 |
