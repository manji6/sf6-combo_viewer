...
- 情報の出し分け（R-1）:
  - **フロー枠**（グラフ内、triage 用）: 種別 ／ `初回行動後 +2F` ／ `受け身:両対応`（バッジ） ／ `リスク` ／ 詳細リンク ＋ ◯強い・×弱い・⚠注意 の各1行
  - **各択カード**（グラフ直下、同ページ）: ＋ `受け身`の全文（その場／後ろの成否） ／ `使いどき` ／ `ガード時` ／ ★
  - **パーツ詳細**: 全項目
...
- **配色は公式 SF6 準拠（U-7）**: 弱＝シアン `--btn-l` / 中＝イエロー `--btn-m` / 強＝レッド `--btn-h`。
  - **OD**（オーバードライブ）はボタン2つ（`PP` / `KK`）だけで表す。色も接頭ラベルも付けない（技名の「OD ○○」で伝わる）
  - **DR / CDR** は緑 `--btn-dr`（`.nt-meta[data-m]` で着色）。「ドライブラッシュ＝緑」
  - **SP**（モダン必殺技ボタン）はオレンジのベタ角丸 `--btn-sp`
  - **AUTO**（モダンのアシストボタン）は濃グレーのピクセル調バッジ `--btn-auto`（旧 `AS`）
  - ライトテーマは各色を暗めに再定義
